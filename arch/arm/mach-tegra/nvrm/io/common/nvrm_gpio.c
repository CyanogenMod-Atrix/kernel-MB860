			       ...
			       NvRmGpioPinHandle *hpins, NvU32 nr_pins)
{
	...
	for (i=0; i<nr_pins; i++) {
		...
		gpio_free(gpio);
		printk(KERN_INFO "pICS_%s: gpio_free(gpio=%i)\n",__func__, gpio);
...

{
	...
	for (i=0; i<nr_pins; i++) {
		...
		printk(KERN_INFO "pICS_%s: gpio_set_value(gpio=%i, v=%i)\n",__func__, gpio, v);
	}
}
			   ...
			   NvU32 nr_pins, NvRmGpioPinMode mode)
{
	...
	for (i=0; i<nr_pins; i++) {
		...
		if (mode == NvRmGpioPinMode_Inactive ||
			...
			printk(KERN_INFO "pICS_%s: tegra_gpio_disable(gpio=%i)\n",__func__, gpio);
		} else {
