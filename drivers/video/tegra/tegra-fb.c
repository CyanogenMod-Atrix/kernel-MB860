...
#ifndef CONFIG_MACH_MOT // Replace with a different copy later in file
int tegra_fb_set_par(struct fb_info *info)
{
...
int tegra_fb_set_par(struct fb_info *info)
{
	/* Need to trigger frames on the qHD display */
	tegra_fb_trigger_frame();
	return 0;
}
#endif
...
#ifdef CONFIG_MACH_MOT
	device_create_file(&d->dev, &dev_attr_screen_size); /* create a sysfs file to advertise screen info */
#endif
